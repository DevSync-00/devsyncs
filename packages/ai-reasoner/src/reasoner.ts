// Mismatch type (standalone - no dependency on CLI package)
...
export class AIReasoner {
  ...
  /**
   * Explain a migration in natural language
   */
  ...
  /**
   * Answer natural language queries about schema
   */
  async query(
    question: string,
    mismatches: Mismatch[],
  ...
  /**
   * Build prompt for migration explanation
   */
...
Provide a clear, helpful answer.`;
  ...
  /**
   * Parse migration explanation from AI response
   */
  ...
  /**
   * Generate template explanation (fallback)
   */
  ...
  /**
   * Generate template risk assessment (fallback)
   */
  private generateTemplateRiskAssessment(mismatches: Mismatch[]): RiskAssessment {
    ...
    return {
      ...
      affectedRecords: 0, // Unknown without database stats
      recommendations: [
