...
function Canvas({ schema, diffs = [], searchQuery = '', relationshipSearch = '', showAdd = true, showRemove = true, showChange = true, layout, onTableClick, onLayoutChange, focusTableId, layoutAlgorithm = 'horizontal' }: Props) {
  ...
  const initialNodes = useMemo<Node<TableNodeData>[]>(() => schema.tables.filter((table) => !hidden.has(tableId(table))).map((table) => {
    ...
    return { id, type: 'table', position: positions[id] || { x: 0, y: 0 }, draggable: !locked.has(id), width: TABLE_WIDTH, height: tableHeight(table.columns.length, isCollapsed), data: { table, color: diffColor || layout?.tableColors?.[id] || table.color || '#22d3ee', collapsed: isCollapsed, locked: locked.has(id), highlighted, relationshipCount: relationCounts.get(table.name) || 0, onToggleCollapse: (nodeId) => toggleCollapseRef.current(nodeId) } };
  ...
  return <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeDoubleClick={(_, node) => onTableClick?.((node.data as TableNodeData).table)} onMoveEnd={onMoveEnd} defaultViewport={layout?.viewport || { x: 0, y: 0, zoom: 1 }} minZoom={0.05} maxZoom={2.5} fitView={!layout?.viewport} fitViewOptions={{ padding: 0.15, maxZoom: 1 }} snapToGrid={layout?.snapToGrid !== false} snapGrid={[16, 16]} selectionMode={SelectionMode.Partial} panOnDrag={[1, 2]} selectionOnDrag multiSelectionKeyCode={['Meta', 'Control']} deleteKeyCode={null} elevateNodesOnSelect onlyRenderVisibleElements colorMode="dark" aria-label="Interactive database schema diagram">
    <Background variant={BackgroundVariant.Lines} gap={24} size={1} color="#353941" /><MiniMap pannable zoomable nodeColor={(node) => (node.data as TableNodeData).color} maskColor="rgba(24,26,30,.72)" className="!border !border-[#515660] !bg-[#25282e]" /><Controls showInteractive={false} className="!border-[#515660] !bg-[#292c32] [&_button]:!border-[#515660] [&_button]:!bg-[#292c32] [&_button]:!fill-zinc-200 [&_button:hover]:!bg-[#393d45]" />
    <button onClick={() => void fitView({ padding: .15, duration: 400 })} className="absolute bottom-4 left-16 z-10 rounded border border-[#555b66] bg-[#292c32] px-2 py-1 font-mono text-[10px] text-zinc-200 shadow-lg hover:border-emerald-400/70 hover:text-emerald-300">Fit</button>
  </ReactFlow>;
}

export function GraphRenderer(props: Props) { return <ReactFlowProvider><div className="h-full w-full bg-[#1b1d21]"><Canvas {...props} /></div></ReactFlowProvider>; }
