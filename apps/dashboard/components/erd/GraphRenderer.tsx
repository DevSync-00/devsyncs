...
function Canvas({ schema, diffs = [], searchQuery = '', relationshipSearch = '', showAdd = true, showRemove = true, showChange = true, layout, onTableClick, onLayoutChange, focusTableId, layoutAlgorithm = 'horizontal' }: Props) {
  ...
  return <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeDoubleClick={(_, node) => onTableClick?.((node.data as TableNodeData).table)} onMoveEnd={onMoveEnd} defaultViewport={layout?.viewport || { x: 0, y: 0, zoom: 1 }} minZoom={0.05} maxZoom={2.5} fitView={!layout?.viewport} fitViewOptions={{ padding: 0.15, maxZoom: 1 }} snapToGrid={layout?.snapToGrid !== false} snapGrid={[16, 16]} selectionMode={SelectionMode.Partial} panOnDrag={[1, 2]} selectionOnDrag multiSelectionKeyCode={['Meta', 'Control']} deleteKeyCode={null} elevateNodesOnSelect onlyRenderVisibleElements colorMode="dark" aria-label="Interactive database schema diagram">
    <Background variant={BackgroundVariant.Dots} gap={20} size={1} color="#334155" /><MiniMap pannable zoomable nodeColor={(node) => (node.data as TableNodeData).color} maskColor="rgba(2,6,23,.75)" className="!border !border-slate-700 !bg-slate-950" /><Controls showInteractive={false} className="!border-slate-700 !bg-slate-950 [&_button]:!border-slate-700 [&_button]:!bg-slate-950 [&_button]:!fill-slate-300" />
    <button onClick={() => void fitView({ padding: .15, duration: 400 })} className="absolute bottom-4 left-16 z-10 rounded border border-slate-700 bg-slate-950 px-2 py-1 font-mono text-[10px] text-slate-300">Fit</button>
  </ReactFlow>;
}

export function GraphRenderer(props: Props) { return <ReactFlowProvider><div className="h-full w-full bg-slate-950"><Canvas {...props} /></div></ReactFlowProvider>; }
